...
function PostStatLikings(props) {
  ...
  const handleLikeChange = async () => {
    try {
      const res = await axios.post(`/api/v1/posts/${postId}/likings`, {
        val: 1,
      });
      if (res.status === 204) {
        setLikeChecked(true);
  ...
  props.isLoggedIn &&
    axios
      .get(`/api/v1/posts/${postId}/likings/user`)
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`/api/v1/posts/${postId}/likings`)
      .then((res) => {
...
export function PostCreator(props) {
  ...
  useEffect(() => {
    props.userName &&
      axios
        .get(`/api/v1/users/${props.userName}`)
        .then((res) => {
